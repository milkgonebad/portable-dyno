...
The original site used to use a component called Tawidget.com which doesn't seem to exist anymore.  This
was a nifty way for the admin to post events, schedule the twitter spam, and display the events on the home page.
No word on what happened to Tawidget.com.  They were probably purchased by Facebook like everyone else.
...
And how about making an admin interface that works on my iPhone?
...
- Make the event ajax friendly and add ui validations - smarten up the HTML 5 date/time - our admin doesn’t use IE so we're safe
- Only display end date if needed on event form
- Add UI form validation via jQuery
